...
from archive.nbp_client import NBPClient
...
Press X to exit"""
        ...
        while True:
            ...
            if option == "x":
                ...
                currency = str(
                    input("Choose currecy code, for example PLN, USD ():")
                    .strip()
                    .upper()
                )
                ...
                currency = str(
                    input(
                        "which currency would you like to SEE? (use format like USD, XXX):"
                    )
                    .strip()
                    .upper()
                )
                ...
                currency = str(
                    input(
                        "Which currency would you like to CHANGE? (use format like USD, XXX):"
                    )
                    .strip()
                    .upper()
                )
                try:
                    amount = float(
                        input(
                            "Enter amount of currency: (positive for deposit, negative for withdrawal)"
                        )
                    )
                    print(
                        Wallet.transfer_funds(user.email, currency, amount, nbp_client)
                    )
                except ValueError:
                ...
                currency = str(
                    input(
                        "Which currency wallet would you like to DELETE? (use format like USD, XXX):"
                    )
                    .strip()
                    .upper()
                )
                ...
                from_currency = (
                    input(
                        "Which wallet do you want to transfer funds FROM? (e.g., PLN, USD): "
                    )
                    .strip()
                    .upper()
                )
                to_currency = (
                    input(
                        "Which wallet do you want to transfer funds TO? (e.g., PLN, USD): "
                    )
                    .strip()
                    .upper()
                )
                try:
                    amount = float(
                        input(
                            f"What amount do you want to transfer from {from_currency} to {to_currency}? "
                        )
                    )
                    print(
                        Wallet.transfer_between_wallets(
                            user.email, from_currency, to_currency, amount, nbp_client
                        )
                    )
                except ValueError:
                    print("Invalid amount. Please enter a number.")
    ...
    def update_user_info(self, user: User):
        ...
        first_name = (
            input(f"First name [{user.first_name}]: ").strip() or user.first_name
        )
        last_name = input(f"Last name [{user.last_name}]: ").strip() or user.last_name
        phone = (
            input(f"Phone number [{user.phone_number or ''}]: ").strip()
        ...
        while True:
            account_type_input = (
                input(f"Account type [{user.account_type}] (personal/business): ")
                .strip()
                .lower()
            )
            if account_type_input == "":
                ...
                print(
                    'Please enter "personal" or "business", or leave blank to keep current value.'
                )
